...
void app_main(void){
    ...
    // Logs contendo diversas informações diferentes.
    ESP_LOGI(TAG, "Versão do framework: %s", esp_get_idf_version());
    ESP_LOGI(TAG, "Modelo do chip: %s", get_chip_model_name(infos.model));
    ...
    // Envia o buffer acima para receber endereço MAC do Wi-Fi.
